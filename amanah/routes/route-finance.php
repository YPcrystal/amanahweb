...
Route::group([
    ...
    Route::middleware(['is-active:1'])->group(function () {
        ...
        // Scholarship
        Route::prefix('data-beasiswa')->name('finance.scholarship-')->group(function () {
            Route::get('/', [StudentScholarshipController::class, 'index'])->name('index');
            Route::post('/{mahasiswa_id}/store', [StudentScholarshipController::class, 'store'])->name('store');
            Route::get('/{code}/update', [StudentScholarshipController::class, 'show'])->name('show');
            Route::delete('/{code}/remove', [StudentScholarshipController::class, 'remove'])->name('remove');
        });
        ...
        Route::prefix('data-pembayaran')->group(function () {
            ...
            Route::get('/unpaid-mahasantri', [PembayaranController::class, 'unpaidMahasantri'])->name('finance.pembayaran-unpaid-mahasantri');
            Route::patch('/unpaid-mahasantri/{code}/{userId}/mark-paid', [PembayaranController::class, 'markAsPaid'])->name('finance.pembayaran-unpaid-mahasantri-mark-paid');
            Route::patch('/unpaid-mahasantri/{code}/{userId}/mark-unpaid', [PembayaranController::class, 'markAsUnpaid'])->name('finance.pembayaran-unpaid-mahasantri-mark-unpaid');
        });
