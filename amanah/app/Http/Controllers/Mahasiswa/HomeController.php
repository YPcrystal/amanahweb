...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $result = $this->applyScholarshipDiscounts($user->id);

        $data['student'] = $result['student'];
        $data['tagihan'] = $result['bills']->sum('finalAmount');

        $data['jadkul'] = JadwalKuliah::where('kelas_id', $data['student']->kelas->first()?->id)->count();
        ...
        // Apply scholarship discount to history items
        $scholarships = $user->scholarships()->get();
        $history->transform(function ($item) use ($scholarships) {
            $applicableScholarship = $scholarships->first(function ($sch) use ($item) {
                return is_null($sch->tagihan_id) || $sch->tagihan_id === $item->tagihan->id;
            });

            if ($applicableScholarship) {
                $discount = ($item->tagihan->price * $applicableScholarship->scholarship_percentage) / 100;
                $item->discounted_price = $item->tagihan->price - $discount;
                $item->scholarship = $applicableScholarship;
            } else {
                $item->discounted_price = $item->tagihan->price;
        ...
        $data['history_sum'] = $data['history']->sum('discounted_price');
        $data['sisatagihan'] = $data['tagihan'] - $data['history_sum'];
    ...
    public function tagihanIndex()
    {
        ...
        $data['student'] = $result['student'];
        $data['tagihan'] = $result['bills'];
        $data['history'] = HistoryTagihan::where('users_id', $user->id)->where('stat', 1)->latest()->get();
    ...
    public function tagihanView($code)
    {
        ...
        if ($checkData !== null) {
            ...
            $tagihan = $result['bills']->firstWhere('code', $code);
    ...
    public function tagihanPayment(Request $request, $code)
    {
        ...
        $amount = $tagihan->price;
        if ($tagihan->scholarship_percentage) {
            $discount = ($tagihan->price * $tagihan->scholarship_percentage) / 100;
            $amount = $tagihan->price - $discount;
        }
        ...
        DB::transaction(function () use ($request, $tagihan, $amount, &$response) {
    ...
    public function tagihanSuccess(Request $request, $code)
    {
        ...
        $amount = $tagihan->tagihan->price;

        $applicableScholarship = $result['bills']->firstWhere('id', $tagihan->tagihan->id);

        if ($applicableScholarship) {
            $discount = ($tagihan->tagihan->price * $applicableScholarship->scholarship_percentage) / 100;
            $amount = $tagihan->tagihan->price - $discount;
        }
