...
class NotifyController extends Controller
{
    ...
    public function roleIndex()
    {
        ...
        if (Auth::guard('dosen')->check()) {
            $data['notify'] = Notification::whereIn('send_to', [0, 2])->latest()->paginate(10);
        } elseif (Auth::guard('mahasiswa')->check()) {
            $data['notify'] = Notification::whereIn('send_to', [0, 3])->latest()->paginate(10);
        } elseif (Auth::check()) {
            // User yang bukan dosen/mahasiswa (misal admin)
            $data['prefix'] = $this->setPrefix(); // ← hanya di sini prefix dipasang
            $data['notify'] = Notification::latest()->paginate(10);
        } else {
            $data['notify'] = collect(); // Kosong kalau tidak login
        }

        return view('user.pages.notifikasi-index', $data);
    }
