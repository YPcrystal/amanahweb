...
@section('content')
    <section class="min-h-screen bg-gray-50 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" x-data="{ activeTab: 'tagihan' }">
            ...
            <div class="mb-8">
                <nav class="flex space-x-8" aria-label="Tabs">
                    <button @click="activeTab = 'tagihan'"
                        :class="activeTab === 'tagihan' ? 'border-[#0C6E71] text-[#0C6E71]' :
                            'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                        class="whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm transition-colors duration-200">
                        Daftar Tagihan
                    </button>
                    <button @click="activeTab = 'history'"
                        :class="activeTab === 'history' ? 'border-[#0C6E71] text-[#0C6E71]' :
                            'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                        class="whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm transition-colors duration-200">
                        Riwayat Tagihan
                    </button>
            ...
            <!-- Tagihan Content -->
            <div x-show="activeTab === 'tagihan'" x-transition:enter="transition ease-out duration-300"
                                ...
                                <div class="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
                                    <div class="space-y-3">
                                        ...
                                        <div class="flex justify-between items-center">
                                            ...
                                            <span
                                                class="text-sm font-semibold text-gray-900 uppercase">{{ $item->code }}</span>
                                        </div>
                                        ...
                                        <div class="space-y-1">
                                            ...
                                            @if ($item->scholarship_percentage > 0)
                                                <div class="flex justify-between items-center text-xs">
                                                    <span class="text-gray-500">Potongan Beasiswa
                                                    </span>
                                                ...
                                                <div class="flex justify-between items-center">
                                                    <span class="text-sm font-medium text-gray-500">Total Bayar</span>
                                                    <span class="font-semibold text-green-600">
                                                        Rp {{ number_format($item->finalAmount, 0, ',', '.') }}
                                                    </span>
                                                </div>
                                            @endif
                                        </div>
                                    ...
                                    @foreach ($tagihan as $key => $item)
                                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center">
                                                {{ ++$key }}</td>
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-center uppercase">
                                                {{ $item->code }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center">
                                                {{ $item->name }}</td>
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center font-semibold">
                                                <div class="font-medium">Rp {{ number_format($item->price, 0, ',', '.') }}
                                                </div>
                                                @if ($item->scholarship_percentage > 0)
                                                    <div class="text-xs text-gray-500">
                                                        Potongan: Rp {{ number_format($item->discount, 0, ',', '.') }}
                                                    </div>
                                                    <div class="text-xs font-semibold text-green-600">
                                                        Dibayar: Rp {{ number_format($item->finalAmount, 0, ',', '.') }}
                                                    </div>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                ...
                                                @else
                                                    <span
                                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">UN-PAID</span>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                <a href="{{ route('mahasiswa.home-tagihan-view', $item->code) }}"
                                ...
                                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                    <div class="space-y-3">
                                        ...
                                        <div class="flex justify-between items-start">
                                            <span class="text-sm font-medium text-gray-500">Kode Tagihan</span>
                                            <span
                                                class="text-sm font-semibold text-gray-900 uppercase">{{ $item->tagihan_code }}</span>
                                        </div>
                                        <div class="flex justify-between items-start">
                                            <span class="text-sm font-medium text-gray-500">Nominal Bayar</span>
                                            <span class="text-sm font-semibold text-gray-900">Rp.
                                                {{ number_format($item->tagihan->price, 0, ',', '.') }}</span>
                                        </div>
                                        <div class="pt-2">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($history as $key => $item)
                                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                                            ...
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-center uppercase">
                                                {{ $item->tagihan_code }}</td>
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center font-semibold">
                                                Rp. {{ number_format($item->tagihan->price, 0, ',', '.') }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                <a href="{{ route('mahasiswa.home-tagihan-invoice', $item->code) }}"
