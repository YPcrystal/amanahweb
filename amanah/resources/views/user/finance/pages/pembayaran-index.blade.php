        ...
        <div class="grid grid-cols-1 gap-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <a href="{{ route($prefix . 'finance.tagihan-index') }}" class="group">
                    <div
                        class="bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 group-hover:shadow-lg">
                ...
                <a href="{{ route($prefix . 'finance.pembayaran-index') }}" class="group">
                    <div
                        ...
                        <div class="p-4 flex flex-col md:flex-row items-center justify-between">
                            ...
                            <div class="text-center md:text-right">
                                <p class="text-2xl font-bold">{{ \App\Models\HistoryTagihan::where('stat', 1)->count() }}
                                </p>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($history as $key => $item)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center uppercase">{{ $item->tagihan_code }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">Rp.
                                        {{ number_format($item->tagihan->price, 0, ',', '.') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <span
                                            class="px-2 py-1 text-sm rounded-full {{ $item->stat === 1 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
