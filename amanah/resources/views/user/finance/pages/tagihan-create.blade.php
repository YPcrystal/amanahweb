    ...
    <section class="p-4 space-y-4">
        ...
        <form action="{{ route($prefix . 'finance.tagihan-store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <div class="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <!-- Class Select -->
                <div class="space-y-1">
                    <label class="block text-sm font-medium text-gray-700">Tagihan Kelas</label>
                    <select name="kelas_id" id="kelas_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                        <option value="0" selected>Pilih Kelas</option>
                        @foreach (\App\Models\Kelas::all() as $kelas)
                            <option value="{{ $kelas->id }}" {{ (isset($selected_kelas_id) && $selected_kelas_id == $kelas->id) ? 'selected' : '' }}>{{ $kelas->name }}</option>
                        @endforeach
                    </select>
                    @error('kelas_id')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-4 whitespace-nowrap text-center">
                                ...
                                @elseif($item->kelas_id !== 0 && $item->kelas)
                                    Kelas<br>
                                    {{ $item->kelas->name }}
                                @else
                                    -
                        ...
                        <div class="sm:flex sm:items-start">
                            <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
                                ...
                                <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div class="space-y-1">
                                        <label class="block text-sm font-medium text-gray-700">Tagihan Kelas</label>
                                        <select name="kelas_id" id="edit_kelas_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                                            <option value="0" selected>Pilih Kelas</option>
                                            @foreach (\App\Models\Kelas::all() as $kelas)
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Remove the onchange handler from kelas_id select
        document.getElementById('kelas_id').removeAttribute('onchange');
        
        // Initialize select2 if you're using it
        if ($().select2) {
            $('#users_id, #prodi_id, #proku_id, #kelas_id').select2({
                placeholder: "Pilih opsi",
                allowClear: true
            });
            
            $('#edit_users_id, #edit_prodi_id, #edit_proku_id, #edit_kelas_id').select2({
                placeholder: "Pilih opsi",
                allowClear: true
            });
        }
