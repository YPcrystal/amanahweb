...
@section('content')
    @php
        ...
        $heroSlider = SiteManage::where('section', 'hero_slider')->where('is_active', '1')->first();
        $founder = SiteManage::where('section', 'founder')->where('is_active', '1')->first();
        $vision = SiteManage::where('section', 'vision')->where('is_active', '1')->first();
        $mission = SiteManage::where('section', 'mission')->where('is_active', '1')->first();
        $facility = SiteManage::where('section', 'facility')->where('is_active', '1')->first();
        $kompetensi = SiteManage::where('section', 'kompetensi')->where('is_active', '1')->first();
        $news = SiteManage::where('section', 'news')->first();
        $youtubeVideo = SiteManage::where('section', 'youtube_video')->where('is_active', '1')->first();
        $activity = SiteManage::where('section', 'activity')->where('is_active', '1')->first();
        $partner = SiteManage::where('section', 'partner')->where('is_active', '1')->first();
    @endphp
    ...
    @if ($heroSlider)
        <section class="relative h-screen overflow-hidden mt-0 lg:mt-0 custom-hero-slider">
            ...
            @foreach ($heroSlider->additional_content as $slide)
            ...
            <div class="custom-slider-nav absolute bottom-8 left-1/2 -translate-x-1/2 flex space-x-2 z-30">
                @foreach ($heroSlider->additional_content as $index => $slide)
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="p-6 lg:p-10">
                        ...
                        <div class="prose prose-lg max-w-none">
                            ...
                            @if (isset($founder->additional_content['quote']) && is_array($founder->additional_content['quote']))
                                @foreach ($founder->additional_content['quote'] as $quote)
                        ...
                        <div class="mt-6">
                            <p class="text-lg font-bold text-gray-800">
                                {{ $founder->additional_content['name'] ?? '' }}</p>
                            <p class="text-emerald-600 font-medium">
                                {{ $founder->additional_content['position'] ?? '' }}</p>
                        </div>
                    </div>
                        ...
                        <div class="relative w-full max-w-sm">
                            <div class="relative group">
                                ...
                                <div
                                    ...
                                    <img src="{{ asset($founder->image_path) }}"
                                        alt="{{ $founder->additional_content['name'] }}"
    ...
    @if ($vision && $mission)
        <section id="visi-misi" class="py-16 px-5 bg-slate-50">
            <div class="container mx-auto text-center">
                ...
                <div class="mb-20">
                    <h3 class="text-3xl font-bold text-teal-700">{{ $vision->title }}</h3>
                ...
                <div>
                    <h3 class="text-3xl font-bold text-teal-700">{{ $mission->title }}</h3>
                    ...
                    <div class="max-w-screen-xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        @foreach ($mission->additional_content['points'] as $point)
                            <div
        ...
        <section id="fasilitas" class="py-20 px-5 bg-gradient-to-br from-gray-50 to-white">
            <div class="max-w-7xl mx-auto">
                <div class="text-center mb-16">
                    <h2 class="text-4xl font-bold text-teal-800 mb-16">{{ $facility->title }}</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach ($facility->additional_content['points'] as $point)
    ...
    @if ($kompetensi)
        <section id="prestasi" class="py-20 px-5 bg-gradient-to-br from-teal-50 to-white">
            <div class="max-w-7xl mx-auto">
                <div class="text-center mb-16">
                    <h2 class="text-4xl font-bold text-teal-800 mb-16">{{ $kompetensi->title }}</h2>
                    <p class="max-w-3xl mx-auto mt-6 text-lg text-gray-600">
                        {{ $kompetensi->content }}
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    @foreach ($kompetensi->additional_content['points'] as $point)
                        <div
    ...
    @if ($news)
        <section class="py-20 px-5 bg-gradient-to-br from-teal-50 to-white">
            <div class="max-w-7xl mx-auto">
                <div class="text-center mb-16">
                    <h2 class="text-4xl font-bold text-teal-800 mb-16">{{ $news->title }}</h2>
                    <p class="max-w-3xl mx-auto mt-6 text-lg text-gray-600">
                        {{ $news->content }}
                    </p>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    @foreach ($news->additional_content['points'] as $point)
                        <div class="bg-white rounded-2xl shadow-lg overflow-hidden hover-scale">
    ...
    @if ($youtubeVideo)
        <section class="py-20 px-5 bg-gradient-to-br from-gray-50 to-white">
            <div class="max-w-7xl mx-auto">
                <div class="text-center mb-16">
                    <h2 class="text-4xl font-bold text-teal-800 mb-16">{{ $youtubeVideo->title }}</h2>
                    <p class="max-w-3xl mx-auto mt-6 text-lg text-gray-600">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    @foreach ($youtubeVideo->additional_content['points'] as $point)
                        <div>
    ...
    @if ($activity)
        <section class="py-20 px-5 bg-gradient-to-br from-gray-50 to-white">
            <div class="max-w-7xl mx-auto">
                <div class="text-center mb-16">
                    <h2 class="text-4xl font-bold text-teal-800 mb-16">{{ $activity->title }}</h2>
                    <p class="max-w-3xl mx-auto mt-6 text-lg text-gray-600">
                        {{ $activity->content }}
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($activity->additional_content['points'] as $point)
    ...
    @if ($partner)
        <section id="support-partner" class="py-16 px-5 bg-gray-100">
            <div class="text-center mb-12 relative">
                <h2 class="text-4xl font-bold text-teal-800 mb-16">{{ $partner->title }}</h2>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-screen-xl mx-auto">
                @foreach ($partner->additional_content['points'] as $point)
                    <div
