                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                    @foreach ($posts as $post)
                        <a href="{{ route('root.post-view', $post->slug) }}" class="news-card">
                            @php
                                $imgPath = $post->image ? 'storage/' . $post->image : 'storage/images/default/default-post.jpg';
                                $imgFull = public_path($imgPath);
                            @endphp
                            <img src="{{ file_exists($imgFull) ? asset($imgPath) : asset('storage/images/default/default-post.jpg') }}" alt="{{ $post->name }}" class="news-image">
